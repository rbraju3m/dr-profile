<x-admin.form-shell :record="$record" :route-name="$routeName" :label="$label">
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2">
            <x-admin.card>
                <div class="space-y-4">
                    <x-admin.bilingual name="question" label="Question" :record="$record" required/>
                    <x-admin.bilingual name="answer" label="Answer" :record="$record" type="textarea" rows="6" required/>
                </div>
            </x-admin.card>
        </div>
        ...
        <x-admin.card>
            <div class="space-y-4">
                <x-admin.select name="group" label="Group" required :value="$record?->group"
