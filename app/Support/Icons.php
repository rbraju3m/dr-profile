...
/**
 * Every glyph the site can draw.
 *
 * These lived inside the icon component, which meant the admin could not offer
 * a choice: the "Icon name" field was free text, and whatever was typed into it
 * — "hero", "admin" — rendered as a bare circle on the public page. The list
 * lives here so the picker can show it and validation can insist on it.
 *
 * Lucide-derived outline paths, kept inline so the site ships no icon font and
 * every glyph inherits currentColor and the surrounding font size.
 */
final class Icons
{
    public const FALLBACK = 'circle';

    /** @return array<string, string> name => SVG path markup */
    ...
    {
        return $name !== null && array_key_exists($name, self::paths());
    }

    /** The named glyph, or the fallback when the name is not one we draw. */
    public static function path(?string $name): string
    {
        $paths = self::paths();

        return $paths[$name] ?? $paths[self::FALLBACK];
    }
}
